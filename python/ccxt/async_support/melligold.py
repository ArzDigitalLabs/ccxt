...
class melligold(Exchange, ImplicitAPI):
    ...
    async def fetch_markets(self, params={}) -> List[Market]:
        ...
        for i in range(0, len(symbols)):
            response = await self.request_with_cookie(self.extend({
                'symbol': symbols[i],
                'format': 'json',
            }, params))
    ...
    async def request_with_cookie(self, params={}):
        response = None
        error = None
        try:
            response = await self.publicGetApiV1ExchangeBuySellPrice(params)
