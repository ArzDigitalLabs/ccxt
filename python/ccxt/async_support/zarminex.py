...
from ccxt.abstract.zarminex import ImplicitAPI
from ccxt.base.types import Any, Market, Strings, Ticker, Tickers
from typing import List


class zarminex(Exchange, ImplicitAPI):

    def describe(self) -> Any:
        return self.deep_extend(super(zarminex, self).describe(), {
            'id': 'zarminex',
            'name': 'Zarminex',
            'countries': ['IR'],
            'rateLimit': 1000,
    ...
    async def fetch_markets(self, params={}) -> List[Market]:
        response = await self.publicGetFunctionUrl(params)
    ...
    def parse_market(self, response) -> Market:
        return {
            ...
            'future': False,
            'option': False,
            'active': self.safe_number(response, 'cached_price') is not None,
            'contract': False,
            'linear': None,
            'inverse': None,
            'contractSize': None,
