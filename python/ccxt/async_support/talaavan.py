...
from ccxt.abstract.talaavan import ImplicitAPI
from ccxt.base.types import Any, Market, Strings, Ticker, Tickers
from typing import List


class talaavan(Exchange, ImplicitAPI):

    def describe(self) -> Any:
        return self.deep_extend(super(talaavan, self).describe(), {
            'id': 'talaavan',
            'name': 'Talaavan',
            'countries': ['IR'],
            'rateLimit': 1000,
    ...
    async def fetch_markets(self, params={}) -> List[Market]:
        response = await self.publicGetV1Price(params)
    ...
    def parse_market(self, response) -> Market:
        return {
            ...
            'future': False,
            'option': False,
            'active': self.safe_bool(response, 'status', False),
            'contract': False,
            'linear': None,
            'inverse': None,
            'contractSize': None,
    ...
    def parse_ticker(self, response, market: Market = None) -> Ticker:
        ...
        price = self.safe_number(data, 'rate')
        if price is not None:
            price = price / 10
        return self.safe_ticker({
            'symbol': market['symbol'],
            'timestamp': self.parse8601(self.safe_string(data, 'date')),
            'datetime': None,
            'high': None,
            'low': None,
            'bid': price,
