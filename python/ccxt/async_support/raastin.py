...
class raastin(Exchange, ImplicitAPI):
    ...
    async def fetch_markets(self, params={}) -> List[Market]:
        ...
        if marketType == 'otc':
            ...
            for i in range(0, len(qoutes)):
                quote = qoutes[i]
                OTCmarkets = await self.publicGetApiV1Market(self.extend(query, {'quote': quote}))
                for j in range(0, len(OTCmarkets)):
                    OTCmarkets[j]['quote'] = quote
                    market = self.parse_otc_market(OTCmarkets[j])
                    result.append(market)
            return result
        response = await self.publicGetApiV1MarketSymbols(query)
    ...
    async def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        for i in range(0, len(tickerSymbols)):
            ...
            marketData = await self.publicGetApiV1MarketSymbolsSymbol(self.extend(request, query))
    ...
    async def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        query = self.omit(params, 'type')
        response = await self.publicGetApiV1MarketSymbolsSymbol(self.extend(request, query))
    ...
    async def fetch_order_book(self, symbol: str, limit: Int = None, params={}) -> OrderBook:
        ...
        response = await self.publicGetApiV1MarketDepthSymbol(request)
        # Response structure:
        # {
