...
class asacoine(Exchange, ImplicitAPI):
    ...
    def fetch_markets(self, params={}):
        ...
        if type == 'otc':
            cumulativeResponse = self.publicGetV1MarketPairsCumulative(request)
            return self.parse_cumulative_markets(cumulativeResponse, type)
        response = self.publicGetV1MarketPairsPricing(request)
    ...
    def fetch_tickers(self, symbols: Strings = None, params={}) -> Tickers:
        ...
        type = self.safe_string(params, 'type', 'spot')
        request = self.omit(params, ['type'])
        self.load_markets(False, {'type': type})
        if symbols is not None:
            symbols = self.market_symbols(symbols)
        response
        if type == 'otc':
            response = self.publicGetV1MarketPairsCumulative(request)
        else:
            response = self.publicGetV1MarketPairsPricing(request)
        if type == 'otc':
