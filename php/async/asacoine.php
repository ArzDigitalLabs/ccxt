...
use ccxt\async\abstract\asacoine as Exchange;
use ccxt\Precise;
use \React\Async;
...
class asacoine extends Exchange {
    ...
    public function fetch_markets($params = array ()) {
        return Async\async(function () use ($params) {
            ...
            if ($type === 'otc') {
                $cumulativeResponse = Async\await($this->publicGetV1MarketPairsCumulative ($request));
                return $this->parse_cumulative_markets($cumulativeResponse);
            }
            $response = Async\await($this->publicGetV1MarketPairsPricing ($request));
    ...
    public function fetch_tickers(?array $symbols = null, $params = array ()): PromiseInterface {
        return Async\async(function () use ($symbols, $params) {
            ...
            $type = $this->safe_string($params, 'type', 'spot');
            $request = $this->omit($params, array( 'type' ));
            Async\await($this->load_markets(false, array( 'type' => $type )));
            if ($symbols !== null) {
                $symbols = $this->market_symbols($symbols);
            }
            $response = array();
            if ($type === 'otc') {
                $response = Async\await($this->publicGetV1MarketPairsCumulative ($request));
            } else {
                $response = Async\await($this->publicGetV1MarketPairsPricing ($request));
            }
            if ($type === 'otc') {
