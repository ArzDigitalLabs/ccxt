...
export default class ourbit extends Exchange {
    ...
    async fetchMarkets (params = {}): Promise<Market[]> {
        ...
        const result = [];
        if (fetchSpot) {
            const response = await (this as any).spotGetApiPlatformSpotMarketV2Tickers (params);
        ...
        if (fetchFutures) {
            const response = await (this as any).futuresGetApiV1ContractTicker (params);
    ...
    parseFutureMarket (market): Market {
    ...
    async fetchTicker (symbol: string, params = {}): Promise<Ticker> {
        ...
        if ((marketType === 'future') || (marketType === 'swap')) {
            const responseData = await (this as any).futuresGetApiV1ContractTicker (this.extend (request, params));
        ...
        const response = await (this as any).spotGetApiPlatformSpotMarketV2SymbolTicker (this.extend (request, params));
    ...
    parseTicker (ticker: Dict, market: Market = undefined, marketType: string = undefined): Ticker {
